  ...
  client.on("interactionCreate", async interaction => {
    ...
    if (interaction.commandName === "crear-personaje") {
      try {
        ...
        if (!isValidUrl(imageUrl) || !isValidUrl(n20Url)) {
          await interaction.reply({
            content: "Las URLs proporcionadas no son válidas. Asegúrate de que tanto la URL de la imagen como la de N20 sean URLs completas y válidas (deben comenzar con http:// o https://)",
    ...
    if (interaction.commandName === "eliminar-personaje") {
      try {
        ...
        const updatedCharacters = await storage.getCharacters(interaction.guildId!);
        const userCharacters = updatedCharacters.filter(c => c.userId === interaction.user.id);
        ...
        const deleteEmbed = new EmbedBuilder()
          ...
          .setTimestamp()
          .addFields({ 
            name: 'Personajes restantes', 
            value: userCharacters.length > 0 
              ? userCharacters.map(c => `- ${c.name}`).join('\n')
              : 'No tienes más personajes.'
          });

        await interaction.reply({ embeds: [deleteEmbed], flags: MessageFlags.Ephemeral });
      } catch (error) {
