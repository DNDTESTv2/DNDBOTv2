...
export default function registerCharacterCommands(
  ...
  client.on("interactionCreate", async interaction => {
    ...
    if (interaction.commandName === "eliminar-personaje") {
      try {
        ...
        if (!success) {
          await interaction.editReply({
            content: "Hubo un error al eliminar el personaje. Por favor, intenta de nuevo."
          });
          return;
        }
  ...
  .setTimestamp();
        ...
        logger.info('Attempting to delete character:', {
          id: character.id,
          name: character.name,
        ...
        const deleted = await storage.deleteCharacter(character.id, guildId);
        ...
        if (!deleted) {
          ...
          await interaction.editReply({
            content: `Hubo un error al eliminar el personaje "${name}". Por favor, intenta de nuevo más tarde.`
          });
          return;
        }
        ...
        logger.error('Error processing delete character command:', error);
        await interaction.editReply({
          content: 'Hubo un error al procesar el comando. Por favor, intenta de nuevo más tarde.'
        });
      }
    }
