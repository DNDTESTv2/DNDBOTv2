  ...
  async deleteCharacter(id: number): Promise<boolean> {
    try {
      ...
      if (!character.guildId || !character.characterId) {
        logger.error('Faltan datos necesarios para eliminar el personaje:', {
          guildId: character.guildId,
          characterId: character.characterId
        });
        return false;
      ...
      // Intentar eliminar el personaje usando las claves correctas
      ...
      logger.info('Ejecutando comando de eliminación:', {
        command: JSON.stringify(deleteCommand, null, 2)
      });

      await docClient.send(deleteCommand);

      logger.info('Personaje eliminado exitosamente');
      return true;
    } catch (error) {
      logger.error('Error al eliminar personaje:', {
